...
def _coco_map_calculation_from_studio(
    y_true_bbox_labels: List[Tuple[tf.RaggedTensor, tf.RaggedTensor]],
    prediction: List[Tuple[tf.RaggedTensor, tf.RaggedTensor]],
    num_classes: int,
):
    ...
    for index, labels in enumerate(y_true_bbox_labels):
        p = prediction[index]
        gt = []
        curr_ps = []
