...
from ei_shared.metrics_utils import allocate_to_bins
...
def profile_model(model_type, model, model_file, validation_dataset, Y_test, X_samples, Y_samples,
    ...
    if mode == 'classification':
        ...
        try:
            ...
            print(f"Failed to calculate regression metrics [{str(e)}]", flush=True)
        ...
        if objdet_details.last_layer != 'fomo':
            ...
            for image, (bboxes, labels) in validation_dataset.take(-1).unbatch():
                y_true_bbox_labels.append((bboxes, labels))
                if width is None:
                    # Derive width and height from first image.
                    if len(image.shape) != 4:
                        raise Exception(f"Expected image to be (1, W, H, C) but was {image.shape}")
                    _single_instance, width, height, _num_channels = image.shape
            ...
            metrics['coco_metrics'] = coco_metrics
...
def get_model_metadata(keras_model, validation_dataset, Y_test, X_samples, Y_samples, has_samples,
    ...
    # standalone full metrics made available for download from dashboard
    metrics = {
        'version': 1,
        'validation': {}
        }
    ...
    if model_float32:
        try:
            ...
            if not recalculate_memory:
                ...
                # Re-compute memory usage if EON RAM optimized memory usage is not present and
                # patch-based inference experiment is enabled.
                if not memory == None and not 'eon_ram_optimized' in memory and patch_based_inference:
                    memory = None
            ...
            if 'metrics' in float32_perf:
                metrics['validation'][model_type] = float32_perf['metrics']
    ...
    if model_int8:
        try:
            ...
            if 'metrics' in int8_perf:
                metrics['validation'][model_type] = int8_perf['metrics']
    ...
    if file_akida:
        ...
        if 'metrics' in akida_perf:
            metrics['validation'][model_type] = akida_perf['metrics']
    ...
    # write metrics if configured to do so
