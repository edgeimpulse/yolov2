...
def get_model_sparsity(model_file, mode, dataset,
    ...
    if mode == 'classification':
        ...
        if objdet_details.last_layer == 'fomo':
            ...
            # dataset shape is [N, 1, W, H, C], too many dimensions for Dense layer (1 to 4 dimensions are supported)
            for item, _ in dataset.take(-1).unbatch():
                # item shape [1, W, H, C], so remove first dimension
                item = process_input(item[0], model.layers[0].input_bits == 4)
                input.append(item)
...
def process_output_yolov2(output_data, img_shape, num_classes, anchors, minimum_confidence_rating=None):
    ...
    if len(pred_boxes) > 0:
        pred_boxes = np.array([[box.y1, box.x1, box.y2, box.x2] for box in pred_boxes])
    else:
        pred_boxes = np.array([[]])
    ...
    nms_scores = object_detection_nms(raw_scores, img_shape[0])
